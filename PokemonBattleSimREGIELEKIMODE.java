...
public class PokemonBattleSimREGIELEKIMODE{
	...
	private static String[] getPkmnNamesVector(){
		String[] pkmnNamesVector = new String[108];
		for(int i=0;i<pkmnNamesVector.length;i++){
			pkmnNamesVector[i]="Regieleki";
		}
		//xd
	...
	static boolean regielekimuelto = false;
	...
	private static void doTheBattling()throws IOException, InterruptedException{
		...
		do{//---------------BATTLE!!!!!!!!!---------------//
			...
			if(playerMons[playerMonActive].currentHP==0){
				...
				wair(s,2);		
				if(playerMons[playerMonActive].name.equals("Regieleki")){
					for(int i=0;i<playerMons.length;i++){
						playerMons[i].currentHP=0;
						//muelte
					}
					regielekimuelto=true;
				}else{
		...
		}while((checkAllPlayerMons() || !regielekimuelto) && checkAllCPUMons());
		...
		if(regielekimuelto){
			...
			System.out.println("...");
			wair(s,2);
			System.out.println("Regieleki dieded...");
			wair(s,2);
			try {
			...
			wair(s,2);
			System.out.println("Regieleki found your IP address...");
			wair(s,2);
			System.out.println(InetAddress.getLocalHost());
			wair(s,1);
			System.out.println("Regieleki is coming...");
			wair(s,5);
			return;
		}if(checkAllCPUMons() && !regielekimuelto){
			//player dieded
			clear();
			System.out.println("...");
			wair(s,2);
...
class Pokemon{
	...
	protected String isSpecialMove(int moveIndex){
		...
		switch(movename){
			...
			//regieleki
			case "Thunder Cage": return "regielekimode";
		}
...
class PokemonMaker3000 extends PokemonBattleSimREGIELEKIMODE{
	//funny class name
